...
# Load local .env in current folder for development only
load_dotenv()

# Use system temp directory for all temporary files to save space and ensure ephemeral storage
...
# Make sure folders exist (in temp dir)
os.makedirs(PARSED_TEXT_OUTPUT_FOLDER, exist_ok=True)
os.makedirs(FAISS_INDEX_OUTPUT_FOLDER, exist_ok=True)
...
async def hackrx_run_api(
    ...
    top_chunks = retrieve_top_k(query_for_embedding, TOP_K_RETRIEVAL, faiss_index, chunk_metadatas, embedding_model)
    rag_prompt = construct_rag_prompt(query, top_chunks, extracted_entities)
